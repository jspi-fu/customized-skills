...
```bash
python .trae/skills/ideological-self-report/scripts/config_manager.py status
```
...
2. **执行扫描**：
   ```bash
   python .trae/skills/ideological-self-report/scripts/config_manager.py check
   ```
...
4. **使用 save-analysis 命令保存分析结果**：
   ```bash
   python .trae/skills/ideological-self-report/scripts/config_manager.py save-analysis \
     --analysis-data '{
       "user_info": {
         "name": "姓名",
         "identity": "身份",
   ...
   **参数说明**：
   - `user_info`: 用户基本信息
   - `word_count`: 各章节字数要求
   - `opening_patterns`: 历史开篇句式列表
   - `historical_weaknesses`: 历史缺点列表，每项包含 theme（主题）、description（描述）、date（时间）
...
2. **使用 setup 命令保存配置**：
   ```bash
   python .trae/skills/ideological-self-report/scripts/config_manager.py setup \
     --name "用户姓名" \
     --identity "用户身份" \
...
3. **验证配置是否保存成功**：
   ...
   检查返回结果中的 `user_info` 字段是否包含正确的用户信息。
...
**更换配置流程：**
```bash
python .trae/skills/ideological-self-report/scripts/config_manager.py reset
```
